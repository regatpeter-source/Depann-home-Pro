...
export const DOCUMENT_TEMPLATE_FIELDS = Object.freeze({
    quote: ["QUOTE_NUMBER", "company.name", "company.address", "company.postalCode", "company.city", "company.phone", "company.email", "company.siret", "company.vat", "company.logo", "client.name", "client.firstName", "client.company", "client.address", "client.postalCode", "client.city", "client.phone", "client.email", "client.siren", "client.vat", "client.interventionAddress", "client.deliveryAddress", "client.adresse_intervention", "client.adresse_livraison", "document.type", "document.number", "document.quoteNumber", "document.author", "document.date", "document.validUntil", "document.serviceDate", "document.purchaseOrderReference", "document.interventionReference", "document.insuranceDossier", "document.mandateNumber", "document.claimNumber", "document.insuredNumber", "document.principal", "document.manager", "document.expert", "document.operationCategory", "document.subject", "document.lines", "document.discountLabel", "document.discount", "document.aids", "document.deposit", "document.subtotal", "document.vat", "document.total", "document.balance", "document.conditions", "document.signature"],
    invoice: ["INVOICE_NUMBER", "company.name", "company.address", "company.postalCode", "company.city", "company.phone", "company.email", "company.siret", "company.vat", "company.logo", "client.name", "client.firstName", "client.company", "client.address", "client.postalCode", "client.city", "client.phone", "client.email", "client.siren", "client.vat", "client.interventionAddress", "client.deliveryAddress", "client.adresse_intervention", "client.adresse_livraison", "document.type", "document.number", "document.invoiceNumber", "document.author", "document.date", "document.dueDate", "document.serviceDate", "document.purchaseOrderReference", "document.interventionReference", "document.insuranceDossier", "document.mandateNumber", "document.claimNumber", "document.insuredNumber", "document.principal", "document.manager", "document.expert", "document.operationCategory", "document.lines", "document.discountLabel", "document.discount", "document.aids", "document.deposit", "document.subtotal", "document.vat", "document.total", "document.balance", "document.status", "document.payment"],
    quitus: ["QUITUS_NUMBER", "company.name", "company.address", "company.phone", "company.email", "company.logo", "client.name", "client.address", "client.city", "client.phone", "client.email", "document.type", "document.number", "document.quitusNumber", "document.technician", "document.intervention", "document.interventionReference", "document.insurance", "document.insuranceDossier", "document.mandateNumber", "document.claimNumber", "document.insuredNumber", "document.principal", "document.manager", "document.expert", "document.date", "document.services", "document.legalText", "document.observations", "document.approval", "document.approved", "document.signature"],
    report: ["REPORT_NUMBER", "company.name", "company.address", "company.phone", "company.email", "company.logo", "client.name", "client.address", "client.phone", "client.email", "document.type", "document.number", "document.reportNumber", "document.interventionNumber", "document.interventionReference", "document.insurance", "document.insuranceDossier", "document.mandateNumber", "document.claimNumber", "document.insuredNumber", "document.principal", "document.manager", "document.expert", "document.technician", "document.date", "document.photos", "document.observations", "document.conclusion", "document.recommendations", "document.signature"]
});
...
export async function renderActiveCustomTemplate(ownerId, documentTypeValue, model) {
    ...
    const attributionField = ["quote", "invoice"].includes(documentTypeValue) ? "document.author" : "document.technician";
    const hasAttributionZone = (selected.definition?.zones || []).some(zone => zone.field === attributionField);
    const attribution = clean(valueAt(model, attributionField), 160);
    if (attribution && !hasAttributionZone) output.buffer = await stampDocumentAttribution(output.buffer, documentTypeValue, attribution);
...
async function stampDocumentAttribution(buffer, type, name) {
    const pdf = await PDFDocument.load(buffer);
    const page = pdf.getPages().at(-1);
    if (!page) return buffer;
    const font = await pdf.embedFont(StandardFonts.Helvetica);
    const label = ["quote", "invoice"].includes(type) ? "Établi par" : "Réalisé par";
    page.drawText(`${label} : ${name}`, { x: 40, y: 18, size: 7, font, color: rgb(.28, .33, .40), maxWidth: Math.max(100, page.getWidth() - 80) });
...
export function buildBillingCustomModel(document, profile = {}) {
    ...
    return {
        ...
        client: { name: clientName, firstName: currentClient.firstName || "", company: currentClient.company || currentClient.companyName || (document.customerType === "Professionnel" ? clientName : ""), address: legal.billingAddress || clientAddress, postalCode: currentClient.postalCode || "", city: currentClient.city || "", phone: currentClient.phone || "", email: currentClient.email || "", siren: legal.customerSiren || "", vat: legal.customerVatNumber || "", interventionAddress: legal.deliveryAddress || "", deliveryAddress: legal.deliveryAddress || "", adresse_intervention: legal.deliveryAddress || "", adresse_livraison: legal.deliveryAddress || "" },
        document: { id: document.id, type: isInvoice ? "FACTURE" : "DEVIS", number: documentNumber, quoteNumber: documentNumber, invoiceNumber: documentNumber, author: document.creatorName || document.createdByName || "", date: document.issueDate || "", validUntil: !isInvoice ? document.dueDate || "" : "", dueDate: isInvoice ? document.dueDate || "" : "", serviceDate: legal.serviceDate || "", purchaseOrderReference: legal.purchaseOrderReference || "", interventionReference: legal.interventionReference || "", insuranceDossier: legal.insuranceDossier || "", mandateNumber: legal.mandateNumber || "", claimNumber: legal.claimNumber || "", insuredNumber: legal.insuredNumber || "", principal: legal.principal || "", manager: legal.manager || "", expert: legal.expert || "", operationCategory: ({ goods: "Livraison de biens", services: "Prestation de services", mixed: "Livraison de biens et prestation de services" })[legal.operationCategory] || "Prestation de services", subject: isInvoice ? "Prestation facturée" : "Proposition de prestation", lines, discountLabel: financial.discountLabel || "Remise", discount: money(discount), aids: aids.map(aid => `${aid.name || "Aide"} : −${aid.calculationMode === "percentage" ? `${aid.amount || 0} %` : money(aid.amount)}${aid.description ? ` · ${aid.description}` : ""}`).join("\n"), deposit: money(deposit), subtotal: money(netHt), vat: money(vat), total: money(netHt + vat), balance: money(Math.max(0, netHt + vat - aidTotal - deposit)), conditions: financial.conditions || document.notes || profile.paymentTerms || "", signature: "", status: document.status || "", payment: [profile.bankIban ? `IBAN : ${profile.bankIban}` : "", profile.bankBic ? `BIC : ${profile.bankBic}` : "", profile.paymentTerms || "", profile.earlyPaymentDiscountTerms || "", profile.latePaymentPenaltyTerms || "", `Indemnité forfaitaire pour frais de recouvrement : ${money(Number(profile.recoveryIndemnityCents ?? 4000) / 100)}`, profile.vatOnDebits ? "TVA acquittée sur les débits." : ""].filter(Boolean).join("\n") }
    };
}
...
export function buildQuitusCustomModel(event, quitus, profile = {}) {
    ...
    const clientName = quitus.signedBy || client.name || event.clientName || "";
    const address = event.location || [client.address, client.postalCode, client.city].filter(Boolean).join(", ") || "";
    const city = client.city || "";
    const approval = "Lu et approuvé – Travaux réalisés et intervention acceptée";
    const legalText = `Je soussigné(e), ${clientName}, reconnais que le technicien de ${profile.companyName || "l’entreprise"} est intervenu à mon domicile / dans les locaux situés ${address}, le ${event.date || ""}, afin de réaliser les travaux et prestations décrits dans le présent document.\n\nJe reconnais que l’intervention s’est déroulée conformément aux travaux convenus et que les prestations indiquées ci-dessus ont été réalisées.\n\nJe déclare avoir pris connaissance des travaux réalisés et les accepter.\n\nLes éventuelles observations ou réserves sont indiquées dans le présent document.\n\nFait le ${event.date || ""}, à ${city}.\n\nSignature du client précédée de la mention : « ${approval} »`;
    return { QUITUS_NUMBER: number, company: companyModel(profile), client: { name: clientName, address, city, phone: client.phone || "", email: client.email || "" }, document: { id: event.id, type: "QUITUS", number, quitusNumber: number, technician: quitus.performedByName || "", date: event.date || "", intervention: event.title || "", interventionReference: client.interventionReference || "", insurance: client.insurance || client.insurer || "", insuranceDossier: client.insuranceDossier || "", mandateNumber: client.mandateNumber || client.mandate || "", claimNumber: client.claimNumber || client.claim || "", insuredNumber: client.insuredNumber || "", principal: client.principal || "", manager: client.manager || client.caseManager || "", expert: client.expert || "", services: event.title || "", observations: quitus.observations || "Aucune observation ni réserve.", approval, approved: quitus.approved === true, legalText, signature: quitus.signature || "" } };
}
