...
export const DOCUMENT_TEMPLATE_FIELDS = Object.freeze({
    quote: ["company.name", "company.address", "company.postalCode", "company.city", "company.phone", "company.email", "company.siret", "company.vat", "company.logo", "client.name", "client.address", "document.number", "document.date", "document.subject", "document.lines", "document.discountLabel", "document.discount", "document.aids", "document.deposit", "document.subtotal", "document.vat", "document.total", "document.balance", "document.conditions", "document.signature"],
    invoice: ["company.name", "company.address", "company.postalCode", "company.city", "company.phone", "company.email", "company.siret", "company.vat", "company.logo", "client.name", "client.address", "document.number", "document.date", "document.dueDate", "document.lines", "document.discountLabel", "document.discount", "document.aids", "document.deposit", "document.subtotal", "document.vat", "document.total", "document.balance", "document.status", "document.payment"],
    quitus: ["company.name", "company.address", "company.phone", "company.email", "company.logo", "client.name", "document.intervention", "document.date", "document.services", "document.observations", "document.signature"],
    report: ["company.name", "company.address", "company.phone", "company.email", "company.logo", "client.name", "client.address", "document.number", "document.insurance", "document.claimNumber", "document.technician", "document.date", "document.photos", "document.observations", "document.conclusion", "document.recommendations", "document.signature"]
});
...
export function registerDocumentTemplateRoutes(app, requireAuthentication) {
    ...
    app.get("/api/document-templates/:documentType", asyncHandler(async (request, response) => {
        ...
        response.json({ documentType: type, fields: DOCUMENT_TEMPLATE_FIELDS[type], templates });
    }));
...
export async function renderActiveCustomTemplate(ownerId, documentTypeValue, model) {
    ...
    const { rows } = await getPool().query(`SELECT template.* FROM depannhome_document_templates template JOIN depannhome_users owner ON owner.id=template.owner_id WHERE template.owner_id=$1 AND template.document_type=$2 AND template.status='active' AND owner.${policyColumn}<>'integrated_only' LIMIT 1`, [ownerId, documentTypeValue]);
    if (!rows[0]) return null;
    const output = await renderCustomDocumentTemplate(rows[0], model);
...
export function buildBillingCustomModel(document, profile = {}) {
    ...
    return {
        ...
        client: { name: document.customerName || "", firstName: "", company: document.customerType === "Professionnel" ? document.customerName || "" : "", address: document.customerAddress || "", postalCode: "", city: "", phone: "", email: "" },
        document: { id: document.id, number: document.documentNumber || "", date: document.issueDate || "", dueDate: document.dueDate || "", subject: document.documentType === "invoice" ? "Prestation facturée" : "Proposition de prestation", lines, discountLabel: financial.discountLabel || "Remise", discount: money(discount), aids: aids.map(aid => `${aid.name || "Aide"} : ${aid.calculationMode === "percentage" ? `${aid.amount || 0} %` : money(aid.amount)}`).join("\n"), deposit: money(deposit), subtotal: money(netHt), vat: money(vat), total: money(netHt + vat), balance: money(Math.max(0, netHt + vat - aidTotal - deposit)), conditions: financial.conditions || document.notes || profile.paymentTerms || "", signature: "", status: document.status || "", payment: [profile.bankIban ? `IBAN : ${profile.bankIban}` : "", profile.bankBic ? `BIC : ${profile.bankBic}` : "", profile.paymentTerms || ""].filter(Boolean).join("\n") }
    };
}

export function buildQuitusCustomModel(event, quitus, profile = {}) {
    return { company: companyModel(profile), client: { name: event.clientName || "", address: event.location || "" }, document: { id: event.id, number: String(event.id || ""), date: event.date || "", intervention: event.title || "", services: event.title || "", observations: event.notes || "", signature: quitus.signature || "" } };
}
...
function defaultDefinition(type, source) {
    const zones = [
        zone("company", "text", "company.name", 40, 35, 245, 35, { fontSize: 14, bold: true }),
...
function sanitizeDefinition(value, type) {
    ...
    const zones = Array.isArray(input.zones) ? input.zones.slice(0, 80).map((item, index) => sanitizeZone(item, index, type)).filter(Boolean) : [];
...
function drawBox(page, zone, size) { if (zone.style.backgroundColor) page.drawRectangle({ x: zone.x, y: size.height - zone.y - zone.height, width: zone.width, height: zone.height, color: pdfColor(zone.style.backgroundColor) }); if (zone.style.borderColor && zone.style.borderWidth) page.drawRectangle({ x: zone.x, y: size.height - zone.y - zone.height, width: zone.width, height: zone.height, borderColor: pdfColor(zone.style.borderColor), borderWidth: zone.style.borderWidth }); }
