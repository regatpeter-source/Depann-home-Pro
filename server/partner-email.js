...
export function registerPartnerEmailRoutes(app, requireAuthentication) {
    ...
    app.put("/api/partner-email/configuration", asyncHandler(async (req, res) => {
        ...
        const old = existing.rows[0] ? decryptElectronicInvoicingCredentials(existing.rows[0].encrypted_credentials) : {};
        ...
        await testMailbox({ provider: "imap", emailAddress: input.emailAddress, credentials, server: input.server });
...
async function testMailbox(connection) { const access = await mailboxAccess({ ...connection, id: 0, encrypted_credentials: encryptElectronicInvoicingCredentials(connection.credentials), server_configuration: connection.server }); const client = new ImapFlow({ host: access.imap.host, port: access.imap.port, secure: access.imap.secure, auth: access.auth, logger: false }); await client.connect(); await client.logout(); const smtp = nodemailer.createTransport({ host: access.smtp.host, port: access.smtp.port, secure: access.smtp.secure, requireTLS: !access.smtp.secure, auth: access.smtpAuth }); await smtp.verify(); }
...
function publicMailError(error) { if (/auth|credential|login|password/i.test(String(error?.message))) return "La boîte a refusé l’authentification. Vérifiez son autorisation ou son mot de passe d’application."; if (/certificate|tls|ssl/i.test(String(error?.message))) return "La connexion sécurisée au serveur de messagerie a échoué."; return "La boîte professionnelle n’a pas pu être synchronisée."; }
