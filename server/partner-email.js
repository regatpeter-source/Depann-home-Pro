...
export async function initializePartnerEmail() {
    ...
    await db.query("ALTER TABLE depannhome_partner_email_messages ADD COLUMN IF NOT EXISTS document_extraction_version INTEGER NOT NULL DEFAULT 0");
...
function oauthErrorLog(error, provider) { return { provider, code: clean(error?.oauthCode || error?.code || "provider_error", 80), errorCodes: Array.isArray(error?.oauthErrorCodes) ? error.oauthErrorCodes : [], status: Number(error?.oauthStatus || error?.statusCode) || 0, correlationId: clean(error?.oauthCorrelationId, 80) }; }
