...
export async function runHealthChecks(database = getPool()) {
    ...
    add({ key: "database_backup", module: "database", status: backupAgeHours !== null && backupAgeHours <= 48 ? "pass" : "warning", severity: "important", message: backupAgeHours === null ? "Aucune preuve de sauvegarde applicative vérifiée. Planifiez npm run db:backup sur un stockage durable ; les sauvegardes natives de l’hébergeur ne sont pas détectées ici." : backupAgeHours <= 48 ? `Dernière sauvegarde applicative vérifiée il y a ${backupAgeHours} h.` : `Dernière sauvegarde applicative vérifiée trop ancienne (${backupAgeHours} h).`, details: { backupAgeHours, evidenceSource: "depannhome_backup_history" } });
