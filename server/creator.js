...
export function registerCreatorRoutes(app, requireCreator, requireAuthentication) {
    ...
    app.get("/api/subscription-change-requests", requireAuthentication, asyncHandler(async (request, response) => {
        ...
        const [requestsResult, accountResult, invoiceResult] = await Promise.all([
            ...
            getPool().query(`SELECT invoice_number AS "invoiceNumber",TO_CHAR(billing_period,'YYYY-MM-DD') AS "billingPeriod",TO_CHAR(issue_date,'YYYY-MM-DD') AS "issueDate",TO_CHAR(due_date,'YYYY-MM-DD') AS "dueDate",amount_cents AS "amountCents",net_amount_cents AS "netAmountCents",status,sent_at AS "sentAt",payment_status AS "paymentStatus",TO_CHAR(paid_date,'YYYY-MM-DD') AS "paidDate",receipt_delivery_status AS "receiptDeliveryStatus" FROM depannhome_subscription_invoices WHERE account_owner_id=$1 AND status<>'cancelled' ORDER BY billing_period DESC,id DESC LIMIT 1`, [request.user.accountOwnerId])
        ]);
