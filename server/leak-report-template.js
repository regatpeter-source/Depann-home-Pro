...
function addSection(pdf, title, values, photos, profile, template, isRecommendations = false, technicianName = "") {
    ...
    for (const [index, observation] of observations.entries()) {
        ...
        y = ensureSpace(pdf, y, 70, profile, template);
        ...
        for (const photo of orderedPhotos(photos.filter(photo => photo.observationId === observation.id))) y = addPhotoWithPage(pdf, photo, y, profile, template);
        y += 10;
    }
