...
function addCover(pdf, report, profile, template, snapshot, photos) {
    ...
    if (exterior) {
        ...
        addPresentationPhoto(pdf, exterior, photoY, 320, Math.min(275, availableHeight), template);
    }
}
...
function addPresentationPhoto(pdf, photo, y, width, height, template) { try { const buffer = dataUrl(photo.dataUrl); const x = 44 + (507 - width) / 2; const padding = 7; pdf.roundedRect(x, y, width, height, 5).fillAndStroke("#f8fafc", "#cbd5e1"); pdf.image(buffer, x + padding, y + padding, { fit: [width - padding * 2, height - padding * 2], align: "center", valign: "center" }); const caption = photoCaption(photo); if (!caption) return y + height; pdf.font(template.font).fillColor("#475569").fontSize(8.5).text(caption, 44, y + height + 7, { width: 507, align: "center", lineGap: 2 }); return pdf.y; } catch { return y; } }
