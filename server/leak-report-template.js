...
function addStepPage(pdf, key, label, values, photos) {
    ...
    if (observations.length) {
        observations.forEach((observation, index) => {
            if (y > 650) { pdf.addPage(); y = 52; }
            pdf.fillColor("#003b73").font("Helvetica-Bold").fontSize(11).text(`Observation ${index + 1}${observation.method ? ` · ${observation.method}` : ""}`, 44, y);
            y += 18;
            y = addField(pdf, "Constat", observation.text, y);
            y = addPhotos(pdf, photos.filter(photo => photo.observationId === observation.id), y);
            y += 8;
        });
        const unlinkedPhotos = photos.filter(photo => !photo.observationId);
        if (unlinkedPhotos.length) y = addPhotos(pdf, unlinkedPhotos, y);
...
function addPhotos(pdf, photos, y) { for (const photo of photos) { if (y > 530) { pdf.addPage(); y = 48; } try { pdf.image(dataUrl(photo.dataUrl), 44, y, { fit: [507, 260] }); } catch { continue; } pdf.fillColor("#475569").font("Helvetica").fontSize(8).text([photo.caption, photo.annotation].filter(Boolean).join(" · ") || photo.name || "Photo", 44, y + 264, { width: 507 }); y += 288; } return y; }
