...
export async function creatorNetworkStatistics() { const { rows } = await getPool().query(`SELECT COUNT(*)::int AS "registeredCompanies",COUNT(*) FILTER (WHERE directory.is_listed AND NOT directory.creator_suspended AND owner.is_active AND NOT owner.is_archived)::int AS "visibleCompanies",COUNT(*) FILTER (WHERE directory.creator_suspended AND NOT owner.is_archived)::int AS "suspendedCompanies",COUNT(*) FILTER (WHERE owner.is_archived)::int AS "archivedCompanies",COUNT(*) FILTER (WHERE directory.accepts_partner_missions AND directory.is_listed AND NOT directory.creator_suspended AND owner.is_active AND NOT owner.is_archived)::int AS "missionReadyCompanies" FROM depannhome_partner_directory directory JOIN depannhome_users owner ON owner.id=directory.owner_id`); const connections = await getPool().query("SELECT COUNT(*)::int AS connected FROM depannhome_partner_connections connection JOIN depannhome_users low_owner ON low_owner.id=connection.company_low_id JOIN depannhome_users high_owner ON high_owner.id=connection.company_high_id WHERE connection.status='connected' AND low_owner.is_active AND NOT low_owner.is_archived AND high_owner.is_active AND NOT high_owner.is_archived"); return { ...(rows[0] || {}), connectedPairs: connections.rows[0]?.connected || 0 }; }
