...
async function notifyAdmins(ownerId, eventType, title, body, payload) { const { rows } = await getPool().query("SELECT id FROM depannhome_users WHERE account_owner_id=$1 AND role IN ('admin','pc_standard','commercial','mobile_admin') AND is_active=TRUE", [ownerId]); await Promise.all(rows.map(row => createNotification(ownerId, row.id, eventType, { entityType: "partner_connection", entityId: String(payload.connectionId || "") }, title, body, payload))); }
