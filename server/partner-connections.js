...
async function createDirectConnectedMission(database, ownerId, connection, value, client) {
    ...
    try {
        ...
        targetClient = await provisionPartnerMissionClient(databaseConnection, targetOwnerId, mapped, { user: { fullName: sourceCompany.name } }, "", { reactivateArchived: true, missionId: mission.id });
...
async function searchDirectory(ownerId, query) {
    ...
    const { rows } = await getPool().query(`
        ...
        WHERE directory.is_listed=TRUE AND directory.creator_suspended=FALSE AND owner.is_active=TRUE AND owner.subscription_tier IN ('basic_plus','pro') AND owner.account_owner_id=owner.id AND owner.id<>$1
...
async function createSharedBillingOutput(document, profile) { const clientResult = document.client_id ? await getPool().query("SELECT client_data AS client FROM depannhome_clients WHERE owner_id=$1 AND client_id=$2", [profile.ownerId, document.client_id]) : { rows: [] }; const { createBillingDocumentOutput } = await import("./billing.js"); return createBillingDocumentOutput({ documentType: document.document_type, documentNumber: document.document_number, creatorName: document.created_by_name || "", customerName: document.customer_name, customerAddress: document.customer_address, clientData: clientResult.rows[0]?.client || null, issueDate: String(document.issue_date).slice(0, 10), dueDate: document.due_date ? String(document.due_date).slice(0, 10) : "", quoteReference: document.quote_reference, lines: document.lines || [], notes: document.notes, financialData: document.financial_data || {}, status: document.status || "" }, profile); }
async function notifyAdmins(ownerId, eventType, title, body, payload) { const { rows } = await getPool().query("SELECT id FROM depannhome_users WHERE account_owner_id=$1 AND role IN ('admin','pc_standard','commercial','mobile_admin') AND is_active=TRUE", [ownerId]); const mission = eventType === "partner_connection_intervention" && positiveId(payload?.missionId); const entityType = mission ? "partner_mission" : "partner_connection"; const entityId = String(mission ? payload.missionId : payload?.connectionId || ""); const action = mission || eventType === "partner_connection_requested" ? "accept" : "view"; const notificationPayload = { ...payload, destination: entityType, action }; await Promise.all(rows.map(row => createNotification(ownerId, row.id, eventType, { entityType, entityId }, title, body, notificationPayload))); }
