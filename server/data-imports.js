...
export function registerDataImportRoutes(app, requireAuthentication) {
    ...
    app.post("/api/data-imports/preview", asyncHandler(async (request, response) => {
        const session = await loadSession(request, request.body?.sessionId); const mapping = sanitizeMapping(request.body?.mapping, session.data_type); const duplicateStrategy = strategy(request.body?.duplicateStrategy);
...
function suggestMapping(columns, dataType) { const aliases = { name: ["nom", "client", "société", "societe", "entreprise"], phone: ["téléphone", "telephone", "mobile", "fixe", "tel"], email: ["mail", "email", "e-mail"], address: ["adresse"], city: ["ville", "commune"], documentNumber: ["numéro", "numero", "référence", "reference", "devis", "facture"], customerName: ["client", "nom", "société", "societe"], issueDate: ["date", "émission", "emission"], description: ["description", "objet", "commentaire", "conclusion"], title: ["titre", "rapport"], reportDate: ["date", "rapport"], quantity: ["quantité", "quantite", "qté", "qte"], unitPrice: ["prix", "montant", "pu"], vatRate: ["tva"], supportedBrands: ["marque"] }; const mapping = {}; TYPE_FIELDS[dataType].forEach(field => { const found = columns.find(column => aliases[field.key]?.some(alias => normalize(column).includes(normalize(alias)))); if (found) mapping[field.key] = found; }); return mapping; }
...
function sanitizeMapping(value, type) { const mapping = value && typeof value === "object" && !Array.isArray(value) ? value : {}; const columns = Object.values(mapping).map(item => clean(item, 80)).filter(Boolean); const required = TYPE_FIELDS[type].filter(field => field.required).map(field => field.key); if (required.some(field => !clean(mapping[field], 80))) return { ok: false, message: "Associez toutes les colonnes obligatoires avant de poursuivre." }; return { ok: true, value: Object.fromEntries(TYPE_FIELDS[type].map(field => [field.key, clean(mapping[field], 80)])) }; }
