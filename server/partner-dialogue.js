...
function requireDialogueAccess(req, res, next) { if (PARTNER_MANAGEMENT_ROLES.has(req.user?.role) || FIELD_ROLES.has(req.user?.role)) return next(); return res.status(403).json({ message: "Le dialogue partenaire n’est pas accessible." }); }
