...
async function updateItemVisibility(req, res) {
    ...
    if (typeof req.body?.partnerVisible !== "boolean") return res.status(400).json({ message: "Visibilité partenaire invalide." });
    if (["quote", "invoice"].includes(item.sourceType) && req.user.role !== "admin") return res.status(403).json({ message: "Le partage des devis et factures est réservé à l’administration." });
