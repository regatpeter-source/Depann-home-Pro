...
export async function initializeTechnicalReports() {
    ...
    `);
    await database.query(`ALTER TABLE depannhome_technical_reports ADD COLUMN IF NOT EXISTS created_by_name VARCHAR(160) NOT NULL DEFAULT '', ADD COLUMN IF NOT EXISTS proofread_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS proofread_by BIGINT REFERENCES depannhome_users(id) ON DELETE SET NULL, ADD COLUMN IF NOT EXISTS proofread_fingerprint VARCHAR(64) NOT NULL DEFAULT '', ADD COLUMN IF NOT EXISTS document_mime_type VARCHAR(150) NOT NULL DEFAULT 'application/pdf'`);
