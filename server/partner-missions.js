        ...
        WHERE mission.owner_id=$1 AND mission.deleted_at IS NULL AND intake.partner_key LIKE 'email-%'
            AND ((COALESCE(email.document_text,'')<>'' AND (LOWER(COALESCE(mission.mapped_data->>'clientName','')) IN ('','client à identifier','client a identifier','client partenaire') OR COALESCE(mission.mapped_data->>'address','')=''))
                ...
                OR ((COALESCE(client.client_data->>'insuranceDossier','')='' OR client.client_data->>'insuranceDossier'=mission.partner_reference)
                    AND COALESCE(email.document_text,'') ~* '(dossier.{0,20}(imh|assur(eur|ance))|nos? r[ée]f[ée]rences?|r[ée]f[ée]rence.{0,20}assur(eur|ance))')
                OR (COALESCE(client.client_data->>'interventionReference','')='' AND COALESCE(email.document_text,'') ~* '(commande|ordre de mission|dossier imh|nos? r[ée]f[ée]rences?)')
...
async function replacePartnerCalendarAssignments(connection, eventId, technicianIds, primaryTechnicianId) {
    ...
    await connection.query("INSERT INTO depannhome_calendar_assignments(event_id,technician_id,is_primary) SELECT $1,technician_id,technician_id=$3::bigint FROM UNNEST($2::bigint[]) AS technician_id", [eventId, technicianIds, primaryTechnicianId]);
}
...
function requireStrictAdministration(req, res, next) { if (isCompanyAdministrator(req)) return next(); return res.status(403).json({ message: "La réactivation d’une mission est réservée au Poste Admin de cette entreprise." }); }
...
function statusLabel(value) {
    return ({ received: "en attente d’acceptation", pending_validation: "en attente d’acceptation", accepted: "acceptée", rejected: "refusée", assigned: "technicien affecté", scheduled: "rendez-vous planifié", en_route: "technicien en route", on_site: "technicien sur site", report_in_progress: "rapport en cours", report_completed: "rapport terminé", report_validated: "rapport validé", quote_sent: "devis envoyé", quote_accepted: "devis accepté", work_completed: "travaux terminés", invoice_sent: "facture envoyée", closed: "clôturée", cancelled: "annulée" })[value] || "statut mis à jour";
}
