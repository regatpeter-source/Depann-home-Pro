...
export async function provisionPartnerMissionClient(connection, ownerId, data, req = {}, linkedClientId = "") {
    ...
    const client = { ...old, id: clientId, isSandbox: Boolean(data.isSandbox || old.isSandbox), name: data.clientName || old.name || "Client partenaire", firstName: data.firstName || old.firstName || "", lastName: data.lastName || old.lastName || "", address: data.address || old.address || "", interventionAddress: data.interventionAddress || old.interventionAddress || data.address || "", postalCode: data.postalCode || old.postalCode || "", city: data.city || old.city || "", phone: data.phone || old.phone || "", email: data.email || old.email || "", insurance: data.insurance || old.insurance || "", principal: data.principal || old.principal || "", claimNumber: data.claimNumber || old.claimNumber || "", insuredNumber: data.insuredNumber || old.insuredNumber || "", expert: data.expert || old.expert || "", manager: data.manager || old.manager || "", gps: data.gps || old.gps || "", notes: mergeText(old.notes, data.description, data.comments), attachments, activityHistory: activity, createdAt: old.createdAt || now, updatedAt: now };
...
export function mapPayload(value) {
    ...
    return {
        ...
        insurance: clean(value.insurance || value.insurer, 160), principal: clean(value.principal || value.orderingParty || value.donor || value.manager, 160), claimNumber: clean(value.claimNumber || value.claim || value.sinisterNumber, 160), insuredNumber: clean(value.insuredNumber || value.memberNumber || value.policyholderNumber, 160), expert: clean(value.expert, 160), manager: clean(value.manager || value.caseManager, 160),
