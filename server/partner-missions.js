...
function mergeAttachments(existing, incoming) { const base = Array.isArray(existing) ? [...existing] : []; const positions = new Map(base.map((attachment, index) => [attachmentSignature(attachment), index])); for (const item of Array.isArray(incoming) ? incoming : []) { if (!/^data:(image\/(jpeg|png|webp)|application\/(pdf|msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document|vnd\.ms-excel|vnd\.openxmlformats-officedocument\.spreadsheetml\.sheet)|text\/plain);base64,[A-Za-z0-9+/=]+$/.test(String(item?.dataUrl || ""))) continue; const attachment = { id: `file-${crypto.randomUUID()}`, type: "Mission partenaire · E-mail", source: "partner_email", sourceAttachmentId: clean(item.sourceAttachmentId, 100), missionId: clean(item.missionId, 100), name: clean(item.name, 255) || "document-partenaire", mime: clean(item.mime, 150), size: Number(item.size) || 0, dataUrl: item.dataUrl, createdAt: new Date().toISOString() }; const signature = attachmentSignature(attachment); const existingIndex = positions.get(signature); if (existingIndex !== undefined) { base[existingIndex] = { ...base[existingIndex], type: attachment.type, source: attachment.source, sourceAttachmentId: attachment.sourceAttachmentId, missionId: attachment.missionId }; continue; } positions.set(signature, base.length); base.push(attachment); } return base.slice(-30); }
