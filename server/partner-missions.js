...
async function receiveMission(req, res) {
    ...
    try {
        ...
        await notifyReceptionAdmins(intake.owner_id, mission.id, mapped, client.created, mission.inserted);
...
async function changeStatus(req, id, status, details) { const ownerId = getAccountOwnerId(req); if (!id) throw clientError(400, "Mission invalide."); const allowedTechnicianStatuses = new Set(["en_route", "on_site", "report_in_progress", "report_completed", "work_completed"]); if (isFieldUser(req) && !allowedTechnicianStatuses.has(status)) throw clientError(403, "Ce statut est réservé à l’administration."); const { rows } = await getPool().query("UPDATE depannhome_partner_missions SET status=$3,updated_at=NOW() WHERE id=$1 AND owner_id=$2 AND ($4::bigint IS NULL OR assigned_technician_id=$4) RETURNING *", [id, ownerId, status, isFieldUser(req) ? req.user.sub : null]); if (!rows[0]) throw clientError(404, "Mission introuvable."); await writeHistory(getPool(), ownerId, id, status, "status_changed", req.user.sub, req.user.role, details, req.ip); await enqueue(getPool(), ownerId, id, "mission_status_changed", { status, ...details }); await recordMissionDialogueEvent({ ownerId, missionId: id, status, action: "status_changed", details, actorName: req.user.fullName || req.user.username }); if (rows[0].assigned_technician_id) await notifyUsers(ownerId, rows[0], rows[0].mapped_data || {}, rows[0].assigned_technician_id, `Mission : ${statusLabel(status)}`); const mission = publicMission(rows[0]); await notifyManagedMissionSource(ownerId, mission, `Mission : ${statusLabel(status)}`); return mission; }
