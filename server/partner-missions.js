...
const STATUS_TRANSITIONS = Object.freeze({
    ...
    accepted: ["assigned", "scheduled", "en_route", "on_site", "closed", "cancelled"],
    assigned: ["scheduled", "en_route", "on_site", "closed", "cancelled"],
    scheduled: ["assigned", "en_route", "on_site", "closed", "cancelled"],
    en_route: ["on_site", "closed", "cancelled"],
    on_site: ["report_in_progress", "report_completed", "work_completed", "invoice_sent", "closed", "cancelled"],
    report_in_progress: ["report_completed", "report_validated", "work_completed", "closed", "cancelled"],
    report_completed: ["report_in_progress", "report_validated", "quote_sent", "work_completed", "invoice_sent", "closed"],
    report_validated: ["quote_sent", "work_completed", "invoice_sent", "closed", "cancelled"],
    quote_sent: ["quote_accepted", "work_completed", "invoice_sent", "closed", "cancelled"],
    quote_accepted: ["work_completed", "invoice_sent", "closed", "cancelled"],
...
export function registerPartnerMissionRoutes(app, requireAuthentication) {
    ...
    app.get("/api/partner-missions/:missionId", asyncHandler(async (req, res) => { const ownerId = getAccountOwnerId(req); const mission = await findMission(ownerId, positiveId(req.params.missionId), req); if (!mission) return res.status(404).json({ message: "Mission introuvable." }); res.json({ mission, history: await history(mission.id), emailAttachments: await missionEmailAttachments(ownerId, mission.id), technicians: canManagePartnerMissions(req) ? await technicians(ownerId) : [] }); }));
...
async function acceptMission(req, id) {
    ...
    try {
        ...
        await assertAvailableSchedule(connection, ownerId, technicianId, schedule, mission.calendar_event_id);
    ...
    const { rows } = await connection.query(`
        ...
        WHERE mission.owner_id=$1 AND mission.deleted_at IS NULL AND intake.partner_key LIKE 'email-%'
            AND ((COALESCE(mission.mapped_data->>'address','')='' AND COALESCE(mission.source_data#>>'{client,address}','')<>'')
                OR (COALESCE(client.client_data->>'insuranceDossier','')='' AND COALESCE(mission.partner_reference,'')<>'')
...
export async function provisionPartnerMissionClient(connection, ownerId, data, req = {}, linkedClientId = "") {
    ...
    const client = { ...old, id: clientId, isSandbox: Boolean(data.isSandbox || old.isSandbox), name: data.clientName || old.name || "Client partenaire", firstName: data.firstName || old.firstName || "", lastName: data.lastName || old.lastName || "", address: data.address || old.address || "", interventionAddress: data.interventionAddress || old.interventionAddress || data.address || "", postalCode: data.postalCode || old.postalCode || "", city: data.city || old.city || "", phone: data.phone || old.phone || "", email: data.email || old.email || "", insurance: data.insurance || old.insurance || "", insuranceDossier: data.insuranceDossier || data.partnerReference || old.insuranceDossier || "", mandateNumber: data.mandateNumber || old.mandateNumber || "", principal: data.principal || old.principal || "", claimNumber: data.claimNumber || old.claimNumber || "", insuredNumber: data.insuredNumber || old.insuredNumber || "", expert: data.expert || old.expert || "", manager: data.manager || old.manager || "", gps: data.gps || old.gps || "", notes: mergeText(old.notes, data.description, data.comments), attachments, activityHistory: activity, createdAt: old.createdAt || now, updatedAt: now };
...
async function assertAvailableSchedule(connection, ownerId, technicianId, schedule, excludedEventId = 0) {
    if (!technicianId || !schedule.date) return;
    await connection.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))", [`partner-schedule:${ownerId}:${technicianId}:${schedule.date}`]);
    const timed = Boolean(schedule.startTime && schedule.endTime);
    const { rows } = await connection.query(`SELECT event.title,TO_CHAR(event.start_time,'HH24:MI') AS "startTime",TO_CHAR(event.end_time,'HH24:MI') AS "endTime" FROM depannhome_calendar_events event WHERE event.owner_id=$1 AND event.event_date=$2::date AND event.id<>$3 AND (event.assigned_technician_id=$4 OR EXISTS(SELECT 1 FROM depannhome_calendar_assignments assignment WHERE assignment.event_id=event.id AND assignment.technician_id=$4)) AND ${timed ? "(event.start_time IS NULL OR event.end_time IS NULL OR (event.start_time<$6::time AND event.end_time>$5::time))" : "TRUE"} ORDER BY event.start_time NULLS FIRST,event.created_at LIMIT 1`, timed ? [ownerId, schedule.date, excludedEventId || 0, technicianId, schedule.startTime, schedule.endTime] : [ownerId, schedule.date, excludedEventId || 0, technicianId]);
    if (rows[0]) throw clientError(409, `Le technicien est déjà affecté à « ${rows[0].title} »${rows[0].startTime && rows[0].endTime ? ` (${rows[0].startTime} – ${rows[0].endTime})` : " (toute la journée)"}. Choisissez un autre créneau.`);
}
async function ensureLeakReport(connection, ownerId, mission, data, clientId, eventId, technicianId) { if (!isLeak(data.interventionType) || !eventId) return null; if (mission.technical_report_id) return mission.technical_report_id; const year = new Date(mission.created_at || Date.now()).getFullYear(); const snapshot = { companyName: "", interventionNumber: mission.intervention_number || `INT-${year}-${String(mission.id).padStart(6, "0")}`, interventionReference: mission.source_mission_number || mission.mission_number || `MP-${year}-${String(mission.id).padStart(6, "0")}`, interventionType: data.interventionType, date: data.date, time: data.startTime, clientName: data.clientName, clientAddress: data.address, clientPhone: data.phone, clientEmail: data.email, technicianName: "", insurance: data.insurance, insuranceDossier: data.insuranceDossier || data.partnerReference || mission.partner_reference || "", mandateNumber: data.mandateNumber || "", claimNumber: data.claimNumber, insuredNumber: data.insuredNumber, principal: data.principal, expert: data.expert, manager: data.manager }; const content = createEmptyLeakContent(snapshot); const { rows } = await connection.query("INSERT INTO depannhome_technical_reports(owner_id,created_by,appointment_id,client_id,report_type,title,report_date,content) VALUES($1,$2,$3,$4,'leak_detection','Rapport de recherche de fuite',$5::date,$6::jsonb) RETURNING id", [ownerId, technicianId || null, eventId, clientId, data.date || new Date().toISOString().slice(0, 10), JSON.stringify(content)]); return rows[0].id; }
...
export function mapPayload(value) {
    ...
    return {
        ...
        insurance: clean(value.insurance || value.insurer, 160), insuranceDossier: clean(value.insuranceDossier || value.partnerReference || value.caseNumber || value.reference, 160), mandateNumber: clean(value.mandateNumber || value.mandate || value.mandat, 160), principal: clean(value.principal || value.orderingParty || value.donor || value.manager, 160), claimNumber: clean(value.claimNumber || value.claim || value.sinisterNumber, 160), insuredNumber: clean(value.insuredNumber || value.memberNumber || value.policyholderNumber, 160), expert: clean(value.expert, 160), manager: clean(value.manager || value.caseManager, 160),
        description: clean(value.description || value.problemDescription || value.breakdownDescription, 2000), comments: clean(value.comments || value.notes, 2000), gps: value.gps && typeof value.gps === "object" ? { latitude: clean(value.gps.latitude, 30), longitude: clean(value.gps.longitude, 30) } : {}, attachments: Array.isArray(value.attachments) ? value.attachments.slice(0, 10) : [], errors: []
    };
}
