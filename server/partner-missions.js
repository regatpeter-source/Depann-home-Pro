...
export async function provisionPartnerMissionClient(connection, ownerId, data, req = {}, linkedClientId = "") {
    ...
    const client = { ...old, id: clientId, isSandbox: Boolean(data.isSandbox || old.isSandbox), name: data.clientName || old.name || "Client partenaire", firstName: data.firstName || old.firstName || "", lastName: data.lastName || old.lastName || "", address: data.address || old.address || "", interventionAddress: data.interventionAddress || old.interventionAddress || data.address || "", postalCode: data.postalCode || old.postalCode || "", city: data.city || old.city || "", phone: data.phone || old.phone || "", email: data.email || old.email || "", insurance: data.insurance || old.insurance || "", insuranceDossier: data.insuranceDossier || old.insuranceDossier || "", mandateNumber: data.mandateNumber || old.mandateNumber || "", principal: data.principal || old.principal || "", claimNumber: data.claimNumber || old.claimNumber || "", insuredNumber: data.insuredNumber || old.insuredNumber || "", expert: data.expert || old.expert || "", manager: data.manager || old.manager || "", gps: data.gps || old.gps || "", notes: mergeText(old.notes, data.description, data.comments), attachments, activityHistory: activity, createdAt: old.createdAt || now, updatedAt: now };
...
async function ensureLeakReport(connection, ownerId, mission, data, clientId, eventId, technicianId) { if (!isLeak(data.interventionType) || !eventId) return null; if (mission.technical_report_id) return mission.technical_report_id; const year = new Date(mission.created_at || Date.now()).getFullYear(); const snapshot = { companyName: "", interventionNumber: mission.intervention_number || `INT-${year}-${String(mission.id).padStart(6, "0")}`, interventionReference: mission.source_mission_number || mission.mission_number || `MP-${year}-${String(mission.id).padStart(6, "0")}`, interventionType: data.interventionType, date: data.date, time: data.startTime, clientName: data.clientName, clientAddress: data.address, clientPhone: data.phone, clientEmail: data.email, technicianName: "", insurance: data.insurance, insuranceDossier: data.insuranceDossier || "", mandateNumber: data.mandateNumber || "", claimNumber: data.claimNumber, insuredNumber: data.insuredNumber, principal: data.principal, expert: data.expert, manager: data.manager }; const content = createEmptyLeakContent(snapshot); const { rows } = await connection.query("INSERT INTO depannhome_technical_reports(owner_id,created_by,appointment_id,client_id,report_type,title,report_date,content) VALUES($1,$2,$3,$4,'leak_detection','Rapport de recherche de fuite',$5::date,$6::jsonb) RETURNING id", [ownerId, technicianId || null, eventId, clientId, data.date || new Date().toISOString().slice(0, 10), JSON.stringify(content)]); return rows[0].id; }
