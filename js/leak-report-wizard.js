...
export function openLeakReportCreation() {
    ...
    dialog.querySelector("[data-report-from-appointment]").addEventListener("click", async () => { dialog.remove(); const { renderCalendar } = await import("./calendar.js?v=216"); renderCalendar({ currentPeriod: true }); });
    dialog.querySelector("[data-create-client-first]").addEventListener("click", async () => { dialog.remove(); const { renderClients } = await import("./clients.js?v=165"); renderClients(); });
}
...
async function completeValidatedReport(validation) {
    ...
    await leaveReport();
    const { synchronizeClients } = await import("./client-sync.js?v=128");
