...
export function openLeakReportCreation() {
    ...
    dialog.querySelector("[data-report-from-appointment]").addEventListener("click", async () => { dialog.remove(); const { renderCalendar } = await import("./calendar.js?v=219"); renderCalendar({ currentPeriod: true }); });
    dialog.querySelector("[data-create-client-first]").addEventListener("click", async () => { dialog.remove(); const { renderClients } = await import("./clients.js?v=166"); renderClients(); });
}
