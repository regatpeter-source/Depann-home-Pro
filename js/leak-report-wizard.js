...
import { acquireReportLock, forceReleaseReportLock, heartbeatReportLock, releaseReportLock } from "./collaboration.js?v=3";
...
export function openLeakReportCreation() {
    ...
    dialog.querySelector("[data-report-from-appointment]").addEventListener("click", async () => { dialog.remove(); const { renderCalendar } = await import("./calendar.js?v=147"); renderCalendar(); });
...
async function validateReport(shell) {
    ...
    const { renderCalendar } = await import("./calendar.js?v=147");
    renderCalendar();
}
...
function moveSection(id, direction) { const order = current.content.sectionOrder; const index = order.indexOf(id); const target = direction === "up" ? index - 1 : index + 1; if (index < 0 || target < 0 || target >= order.length) return; [order[index], order[target]] = [order[target], order[index]]; }
