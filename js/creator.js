...
async function renderCreatorSupportRequests() {
    ...
    workspace.innerHTML = `<section class="creator-form"><div class="form-heading"><div><p class="eyebrow">Entreprises internes</p><h3>Demandes Support</h3></div><span class="creator-state">${requests.length} demande${requests.length > 1 ? "s" : ""}</span></div><div class="creator-network-list">${requests.length ? requests.map(item => `<form class="creator-network-company" data-support-request="${escapeHtml(item.id)}"><div><strong>${escapeHtml(item.subject || item.companyName || item.senderName || item.senderUsername)}</strong>${item.category ? `<span class="creator-state">${escapeHtml(item.category)}</span>` : ""}<p>${escapeHtml(item.message)}</p>${item.technicalContext && Object.keys(item.technicalContext).length ? `<details><summary>Contexte du poste</summary><p>${Object.entries(item.technicalContext).map(([key, value]) => `${escapeHtml(key)} : ${escapeHtml(value)}`).join(" · ")}</p></details>` : ""}<small>${escapeHtml(item.companyName || item.senderName || item.senderUsername)} · ${escapeHtml(formatDateTime(item.createdAt))}${item.senderEmail ? ` · ${escapeHtml(item.senderEmail)}` : ""}</small></div><div class="creator-form-actions"><select name="status">${["new", "under_review", "answered", "closed"].map(status => `<option value="${status}" ${item.status === status ? "selected" : ""}>${supportRequestStatusLabel(status)}</option>`).join("")}</select><input name="creatorNote" maxlength="2000" value="${escapeHtml(item.creatorNote || "")}" placeholder="Note interne"><button class="secondary-button">Enregistrer</button><button type="button" class="primary-button" data-request-assistance="${escapeHtml(item.id)}">Demander une assistance</button></div></form>`).join("") : '<p class="muted">Aucune demande Support.</p>'}</div></section>`;
...
async function renderCreatorAssistanceSession(sessionId) {
    ...
    const workstationMembers = (diagnostics.members || []).filter(member => ["admin", "pc_standard", "commercial"].includes(member.role));
    ...
    workspace.innerHTML = `<section class="creator-form creator-assistance-center">
        ...
        <section class="creator-assistance-section"><div class="form-heading"><div><p class="eyebrow">Postes PC</p><h4>Accès et double authentification</h4></div></div><div class="creator-network-list">${workstationMembers.length ? workstationMembers.map(member => `<article class="creator-network-company"><div><strong>${escapeHtml(member.fullName || member.username)}</strong><p>${escapeHtml(creatorMemberRoleLabel(member.role))} · ${escapeHtml(member.username)} · ${member.activeAuthenticators} authentificateur(s)</p></div><div class="creator-form-actions"><span class="creator-state${member.isActive ? "" : " suspended"}">${member.isActive ? "Actif" : "Désactivé"}</span>${member.role !== "admin" || member.isActive || String(member.id) === String(company.id) ? "" : `<button type="button" class="secondary-button" data-assistance-action="reactivate_administrator" data-target-id="${escapeHtml(member.id)}">Réactiver</button>`}${Number(member.activeAuthenticators) > 0 ? `<button type="button" class="secondary-button danger-button" data-assistance-action="reset_administrator_2fa" data-target-id="${escapeHtml(member.id)}">Réinitialiser 2FA</button>` : '<span class="creator-state archived">2FA non activée</span>'}</div></article>`).join("") : '<p class="auth-message error">Aucun Poste PC compatible trouvé.</p>'}</div></section>
...
function renderOrganizationFields(organization = {}, maxGroupCompanies = 1) {
    ...
    return `
        <fieldset class="creator-subscription-fields"><legend>Organisation et interface</legend>
            ...
            <div class="form-grid">
                <label>Type d’interface<select name="organizationInterfaceType"><option value="partner" ${interfaceType === "partner" ? "selected" : ""}>Compte Partenaire gratuit — sans abonnement</option><option value="standard" ${interfaceType === "standard" ? "selected" : ""}>Interface Standard — modules selon Basic / Basic+ / Pro</option><option value="group" ${interfaceType === "group" ? "selected" : ""}>Groupe / Multi-entreprises — inclus dans Pro</option></select></label>
