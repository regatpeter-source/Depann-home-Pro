...
let supportActivities = [];
let observedSupportDialog = null;

const SUPPORT_ROUTE_LABELS = Object.freeze({ home: "Accueil", clients: "Clients", billing: "Devis et factures", accounting: "Comptabilité", "partner-missions": "Missions partenaires", "company-email": "E-mail entreprise", calendar: "Planning", "technical-reports": "Rapports techniques", settings: "Paramètres" });
...
export function initializeCollaboration() {
    ...
    document.addEventListener("change", emitSupportFieldChange, { capture: true });
    ...
    new MutationObserver(emitSupportDialogOpening).observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["open", "hidden", "aria-hidden"] });
}
...
async function loadCompanyAssistancePresence() {
    ...
    if (!session) { activeAssistanceSessionId = ""; followingSupport = false; supportActivities = []; removeSupportOverlay(); removeSupportActivityPanel(); return; }
    const nextSessionId = String(session.id || "");
    if (nextSessionId !== activeAssistanceSessionId) { followingSupport = true; supportActivities = []; }
    activeAssistanceSessionId = nextSessionId;
    const banner = document.createElement("aside");
    banner.id = "companySupportPresenceBanner";
    banner.className = "support-control-banner company-support-presence-banner";
    banner.innerHTML = `<div><strong>Support Depann’Home Pro connecté à distance</strong><span>Ses pages et actions sont affichées en direct ci-dessous.</span><small>Accès autorisé jusqu’au ${escapeHtml(formatDateTime(session.expiresAt))} · ${escapeHtml(session.reason || "Assistance à distance")}</small></div><div class="support-presence-actions"><button type="button" class="secondary-button" data-follow-company-support>${followingSupport ? "Arrêter le suivi automatique" : "Suivre automatiquement"}</button>${result.data?.canRevoke ? `<button type="button" class="secondary-button danger-button" data-manage-company-assistance="${escapeHtml(session.id)}">Gérer ou retirer l’accès</button>` : '<strong>Accès supervisé par votre Poste Admin</strong>'}</div>`;
    document.body.prepend(banner);
    banner.querySelector("[data-manage-company-assistance]")?.addEventListener("click", () => openCompanyAssistanceRequest(session.id));
    banner.querySelector("[data-follow-company-support]")?.addEventListener("click", event => { followingSupport = !followingSupport; event.currentTarget.textContent = followingSupport ? "Arrêter le suivi automatique" : "Suivre automatiquement"; });
    renderSupportActivityPanel();
}
...
export async function markPartnerNotificationRead(id) { if (!id) return; const result = await request("/api/collaboration/notifications/read", { method: "POST", body: JSON.stringify({ ids: [id], scope: "partner" }) }); if (!result.ok) return; partnerNotifications = partnerNotifications.map(item => String(item.id) === String(id) ? { ...item, readAt: new Date().toISOString() } : item); renderPartnerNotificationBadge(); }
...
function emitSupportClick(event) { if (document.body.dataset.supportControl !== "true" && followingSupport) stopFollowingSupport(); if (event.target?.closest?.("input, textarea, select, [contenteditable=true], [data-sensitive]")) { sendCobrowse({ type: "field", route: currentRoute() }); return; } const interactive = event.target?.closest?.("button, a, [role=button], [role=link]"); const action = interactive?.matches?.("a, [role=link]") ? "link" : interactive ? "button" : "control"; sendCobrowse({ type: "click", route: currentRoute(), x: event.clientX / Math.max(window.innerWidth, 1), y: event.clientY / Math.max(window.innerHeight, 1) }); sendCobrowse({ type: "action", route: currentRoute(), action }); }
function emitSupportFieldChange(event) { if (!event.target?.matches?.("input, textarea, select, [contenteditable=true]")) return; sendCobrowse({ type: "field", route: currentRoute() }); }
function emitSupportDialogOpening() { if (document.body.dataset.supportControl !== "true") return; const dialog = [...document.querySelectorAll("dialog[open], [role=dialog], [aria-modal=true]")].find(element => !element.hidden && element.getClientRects().length > 0 && getComputedStyle(element).visibility !== "hidden"); if (!dialog) { observedSupportDialog = null; return; } if (dialog === observedSupportDialog) return; observedSupportDialog = dialog; sendCobrowse({ type: "dialog", route: currentRoute() }); }
...
function handleSupportCobrowse(data) {
    ...
    const description = describeSupportActivity(data);
    if (description) appendSupportActivity(description, data.at);
}
function describeSupportActivity(data) { const area = SUPPORT_ROUTE_LABELS[data.route || data.area] || "Application"; if (data.type === "route") return `Page « ${area} » ouverte`; if (data.type === "action") return `${data.action === "link" ? "Lien ouvert" : data.action === "button" ? "Bouton activé" : "Commande utilisée"} dans « ${area} »`; if (data.type === "field") return `Champ utilisé dans « ${area} » (contenu masqué)`; if (data.type === "dialog") return `Fenêtre ouverte dans « ${area} »`; if (data.type === "operation") return `${data.operation === "write" ? "Modification" : "Consultation"} ${data.outcome === "success" ? "réussie" : "refusée ou échouée"} dans « ${area} »`; return ""; }
function appendSupportActivity(description, at) { const previous = supportActivities.at(-1); const timestamp = at || new Date().toISOString(); if (previous?.description === description && new Date(timestamp).getTime() - new Date(previous.at).getTime() < 1_000) return; supportActivities.push({ description, at: timestamp }); supportActivities = supportActivities.slice(-50); renderSupportActivityPanel(); }
function renderSupportActivityPanel() { if (!activeAssistanceSessionId) return; let panel = document.getElementById("companySupportActivity"); if (!panel) { panel = document.createElement("aside"); panel.id = "companySupportActivity"; panel.className = "support-activity-panel"; panel.setAttribute("aria-label", "Actions du Support en direct"); document.body.append(panel); } panel.innerHTML = `<header><div><strong>Actions du Support en direct</strong><small>${supportActivities.length ? `${supportActivities.length} action${supportActivities.length > 1 ? "s" : ""} visible${supportActivities.length > 1 ? "s" : ""}` : "En attente de la première action…"}</small></div><span class="support-live-indicator">En direct</span></header><ol aria-live="polite">${supportActivities.length ? [...supportActivities].reverse().map(item => `<li><time>${escapeHtml(formatActivityTime(item.at))}</time><span>${escapeHtml(item.description)}</span></li>`).join("") : '<li class="support-activity-empty">Les ouvertures de pages, commandes et opérations apparaîtront ici sans afficher les données saisies.</li>'}</ol>`; }
function removeSupportActivityPanel() { document.getElementById("companySupportActivity")?.remove(); }
function formatActivityTime(value) { const date = new Date(value); return Number.isNaN(date.getTime()) ? "" : new Intl.DateTimeFormat("fr-FR", { hour: "2-digit", minute: "2-digit", second: "2-digit" }).format(date); }
...
function stopFollowingSupport() { followingSupport = false; const button = document.querySelector("[data-follow-company-support]"); if (button) button.textContent = "Suivre automatiquement"; }
