...
function renderOverview(panel, profilePanel) {
    ...
    panel.querySelector("[data-billing-action=open-purchases]")?.addEventListener("click", async () => {
        const { renderPurchases } = await import("./purchases.js?v=128");
        renderPurchases();
    });
...
function renderDocumentAids(panel, billingDocument, rerender) {
    ...
    const deposit = billingDocument.financialData;
    const depositBlock = `<fieldset class="accounting-aid-fieldset billing-deposit-fieldset"><legend>Acompte déjà encaissé</legend><p class="muted">À renseigner avant l’émission définitive. L’acompte sera visible sur le PDF et déduit du solde restant, sans réduire le total facturé.</p><div class="form-grid"><label>Montant encaissé (€)<input type="number" min="0" step="0.01" data-deposit-amount value="${escapeHtml(deposit.depositAmount || 0)}"></label><label>Date d’encaissement<input type="date" data-deposit-date value="${escapeHtml(deposit.depositDate || "")}"></label><label>Mode d’encaissement<select data-deposit-method><option value="">Non renseigné</option>${["Carte bancaire", "Chèque", "Espèces", "Virement", "Autre"].map(method => `<option ${deposit.depositMethod === method ? "selected" : ""}>${method}</option>`).join("")}</select></label><label>Référence / preuve<input maxlength="160" data-deposit-reference placeholder="N° transaction, chèque…" value="${escapeHtml(deposit.depositReference || "")}"></label></div></fieldset>`;
    panel.innerHTML = `<div class="form-heading"><div><p class="eyebrow">Primes, aides, franchise et acompte</p><h3>Lignes déduites du solde à payer</h3><p class="muted">Aucune déduction n’est ajoutée automatiquement. Chaque montant sélectionné reste visible sur le PDF.</p></div></div>${depositBlock}${deductibleBlock}${aids.length ? `<fieldset class="accounting-aid-fieldset">${aids.map(aid => `<label><input type="checkbox" value="${escapeHtml(aid.id)}" ${selectedAids.some(item => item.aidType !== "insurance_deductible" && item.name === aid.name) ? "checked" : ""}> ${escapeHtml(aid.name)} · ${aid.calculationMode === "percentage" ? `${formatNumber(aid.amount)} %` : formatMoney(aid.amount)}</label>`).join("")}</fieldset>` : '<p class="muted">Aucune prime configurée. Ajoutez-les depuis « Gérer les lignes et aides ».</p>'}`;
    panel.querySelector("[data-deposit-amount]").addEventListener("input", event => { billingDocument.financialData.depositAmount = Math.max(0, Number(event.currentTarget.value) || 0); rerender(); });
    panel.querySelector("[data-deposit-date]").addEventListener("change", event => { billingDocument.financialData.depositDate = event.currentTarget.value; rerender(); });
    panel.querySelector("[data-deposit-method]").addEventListener("change", event => { billingDocument.financialData.depositMethod = event.currentTarget.value; rerender(); });
    panel.querySelector("[data-deposit-reference]").addEventListener("input", event => { billingDocument.financialData.depositReference = event.currentTarget.value; rerender(); });
...
function renderTotals(panel, lines, financialData = {}) {
    const grossHt = lines.reduce((total, line) => total + lineTotal(line), 0);
    const grossVat = lines.reduce((total, line) => total + lineTotal(line) * (Number(line.vatRate) || 0) / 100, 0);
    const discountAmount = Math.min(grossHt, financialData.discountMode === "percentage" ? grossHt * Number(financialData.discountAmount || 0) / 100 : Number(financialData.discountAmount || 0));
    const totalHt = grossHt - discountAmount;
    const totalVat = grossHt ? grossVat * totalHt / grossHt : 0;
    ...
    panel.innerHTML = `<span>Total HT <strong>${formatMoney(grossHt)}</strong></span>${discountAmount ? `<span>Remise HT <strong>− ${formatMoney(discountAmount)}</strong></span>` : ""}<span>TVA <strong>${formatMoney(totalVat)}</strong></span><span>Total TTC <strong>${formatMoney(totalTtc)}</strong></span>${aidAmount ? `<span>Primes / aides / franchise <strong>− ${formatMoney(aidAmount)}</strong></span>` : ""}${depositAmount ? `<span>Acompte déjà encaissé <strong>− ${formatMoney(depositAmount)}</strong></span>` : ""}${aidAmount || depositAmount ? `<span>Solde à payer <strong>${formatMoney(Math.max(0, totalTtc - aidAmount - depositAmount))}</strong></span>` : ""}`;
}
...
function emptyFinancialData() { return { discountMode: "fixed", discountAmount: 0, depositAmount: 0, depositDate: "", depositMethod: "", depositReference: "", conditions: "", comments: "", aids: [] }; }
function normalizeFinancialData(value) { return { ...emptyFinancialData(), discountMode: value?.discountMode === "percentage" ? "percentage" : "fixed", discountAmount: Number(value?.discountAmount) || 0, depositAmount: Number(value?.depositAmount) || 0, depositDate: value?.depositDate || "", depositMethod: value?.depositMethod || "", depositReference: value?.depositReference || "", conditions: value?.conditions || "", comments: value?.comments || "", aids: Array.isArray(value?.aids) ? value.aids.filter(aid => aid?.name).map(toAidSnapshot) : [] }; }
