...
export async function renderPartnerMissions(options = {}) {
    ...
    const targetedMission = options.missionId ? dashboard.missions.find(item => String(item.id) === String(options.missionId)) : null;
    if (targetedMission) {
    ...
    if (activeMissionSpace === "email") {
        ...
        if (counters[0]) { counters[0].querySelector("span").textContent = "À confirmer / accepter"; counters[0].querySelector("strong").textContent = dashboard.partnerEmail.candidates.length + emailMissions.filter(mission => ["received", "pending_validation"].includes(mission.status)).length; }
    ...
    if (options.missionId) {
        if (options.sourceDialogue) await openPartnerDialogue(options.missionId, { sourceDialogue: true });
        else if (options.action === "accept" && targetedMission && ["received", "pending_validation"].includes(targetedMission.status) && canManagePartnerMissions()) await openPartnerMissionPlanning(targetedMission);
        else await showDetail(options.missionId);
    }
}
...
async function openPartnerMissionPlanning(mission) {
    const { renderCalendar } = await import("./calendar.js?v=232");
...
function labelStatus(value) { return ({ email_candidate: "À confirmer", received: "À accepter", pending_validation: "À accepter", accepted: "Acceptée", rejected: "Refusée", assigned: "Affectée", scheduled: "Planifiée", en_route: "En route", on_site: "Sur site", report_in_progress: "Rapport en cours", report_completed: "Rapport terminé", report_validated: "Rapport validé", quote_sent: "Devis envoyé", quote_accepted: "Devis accepté", work_completed: "Travaux terminés", invoice_sent: "Facture envoyée", closed: "Clôturée", cancelled: "Annulée" })[value] || "Statut non renseigné"; }
