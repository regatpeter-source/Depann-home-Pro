...
import { openPartnerDialogue } from "./partner-dialogue.js?v=6";
...
function renderNewMissionEntry(node) {
    ...
    let step = 1;
    const values = { connectionId: preferredConnectionId || String(connections[0].id), clientId: "", clientMode: "existing", subject: "", interventionType: "", priority: "normal", comments: "", requestedDate: new Date().toISOString().slice(0, 10), keepInOwnCalendar: false, sharedAttachmentIds: [] };
    const render = () => {
...
async function openNetworkMissionPlanning(mission) {
    const { renderCalendar } = await import("./calendar.js?v=156");
