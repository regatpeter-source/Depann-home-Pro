...
function renderFinancialEditor(node, document) {
    ...
    panel.querySelector("form").addEventListener("submit", async event => {
        ...
        const result = await api(`/api/accounting/documents/${document.id}/financial-data`, { method: "PUT", body: JSON.stringify({ discountMode: form.get("discountMode"), discountAmount: form.get("discountAmount"), depositAmount: form.get("depositAmount"), depositDate: form.get("depositDate"), depositMethod: form.get("depositMethod"), depositReference: form.get("depositReference"), conditions: form.get("conditions"), comments: form.get("comments"), aids }) });
...
function renderSalesJournal(node) {
    ...
    node.innerHTML = `<section class="accounting-panel"><div class="form-heading"><div><p class="eyebrow">Grand livre persistant</p><h3>Journal des ventes</h3><p class="muted">Une seule écriture numérotée par facture ou avoir, avec le détail Débit / Crédit.</p></div>${isAccountingReadOnly() ? "" : '<button type="button" class="secondary-button" id="showInvoices">Factures à comptabiliser</button>'}</div>${renderLedgerEntries(entries)}</section>`;
...
function renderLegacyElectronic(node) {
    ...
    node.querySelectorAll("[data-transmit]").forEach(button => button.addEventListener("click", async () => { button.disabled = true; const result = await api(`/api/accounting/e-invoicing/documents/${button.dataset.transmit}/transmit`, { method: "POST" }); if (!result.ok) alert(result.message || "Transmission impossible."); renderAccounting("electronic"); }));
}
...
function renderLegacySettings(node) {
    ...
    planFieldset?.querySelector('[name="customerAccount"]')?.closest("label")?.insertAdjacentHTML("afterend", `<label>Compte avances / acomptes reçus<input name="customerAdvanceAccount" value="${escapeHtml(settings.chartConfig.customerAdvanceAccount || "419100")}"></label>`);
    planFieldset?.querySelector('[name="customerAccount"]')?.closest("label")?.insertAdjacentHTML("afterend", `<label>Compte aides / franchises à recevoir<input name="aidReceivableAccount" value="${escapeHtml(settings.chartConfig.aidReceivableAccount || "467000")}"></label>`);
    planFieldset?.querySelector('[name="bankAccount"]')?.closest("label")?.insertAdjacentHTML("afterend", `<label>Compte caisse<input name="cashAccount" value="${escapeHtml(settings.chartConfig.cashAccount || "530000")}"></label>`);
    const journals = settings.journalConfig || {};
    if (planFieldset) planFieldset.insertAdjacentHTML("afterend", `<fieldset class="accounting-rules form-wide"><legend>Journaux et exercice</legend><label>Code journal des ventes<input name="salesJournalCode" maxlength="10" value="${escapeHtml(journals.sales?.code || "VE")}"></label><label>Libellé ventes<input name="salesJournalLabel" maxlength="100" value="${escapeHtml(journals.sales?.label || "Ventes")}"></label><label>Code journal de banque<input name="bankJournalCode" maxlength="10" value="${escapeHtml(journals.bank?.code || "BQ")}"></label><label>Libellé banque<input name="bankJournalLabel" maxlength="100" value="${escapeHtml(journals.bank?.label || "Banque")}"></label><label>Code journal de caisse<input name="cashJournalCode" maxlength="10" value="${escapeHtml(journals.cash?.code || "CA")}"></label><label>Libellé caisse<input name="cashJournalLabel" maxlength="100" value="${escapeHtml(journals.cash?.label || "Caisse")}"></label><label>Code opérations diverses<input name="generalJournalCode" maxlength="10" value="${escapeHtml(journals.general?.code || "OD")}"></label><label>Libellé opérations diverses<input name="generalJournalLabel" maxlength="100" value="${escapeHtml(journals.general?.label || "Opérations diverses")}"></label><label>Début d’exercice<input name="fiscalYearStart" type="date" value="${escapeHtml(settings.fecConfig?.fiscalYearStart || "")}"></label><label>Clôture d’exercice<input name="fiscalYearEnd" type="date" value="${escapeHtml(settings.fecConfig?.fiscalYearEnd || "")}"></label><p class="muted form-wide">Les comptes proposés sont des valeurs initiales configurables, pas des règles universelles. Faites-les valider par votre cabinet comptable.</p></fieldset>`);
    ...
    node.querySelector("form").addEventListener("submit", async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const payload = { provider: form.get("provider"), identifier: form.get("identifier"), apiKey: form.get("apiKey"), enabled: form.get("enabled") === "on", chartConfig: Object.fromEntries(["salesAccount", "customerAccount", "customerAdvanceAccount", "aidReceivableAccount", "bankAccount", "cashAccount", "vatCollectedAccount", "purchaseAccount", "supplierAccount"].map(key => [key, form.get(key)])), journalConfig: { sales: { code: form.get("salesJournalCode"), label: form.get("salesJournalLabel"), active: true }, bank: { code: form.get("bankJournalCode"), label: form.get("bankJournalLabel"), active: true }, cash: { code: form.get("cashJournalCode"), label: form.get("cashJournalLabel"), active: true }, general: { code: form.get("generalJournalCode"), label: form.get("generalJournalLabel"), active: true } }, fecConfig: { fiscalYearStart: form.get("fiscalYearStart"), fiscalYearEnd: form.get("fiscalYearEnd") }, aidEngineConfig: { enabled: form.get("aidEngineEnabled") === "on", source: form.get("aidEngineSource") } }; const result = await api("/api/accounting/settings", { method: "PUT", body: JSON.stringify(payload) }); if (!result.ok) return alert(result.message || "Enregistrement impossible."); renderAccounting("settings"); });
}
...
async function renderElectronic(node) {
    ...
    const activeTransmissionDocumentIds = new Set(transmissions.filter(item => ["queued", "sent", "accepted"].includes(item.status)).map(item => String(item.documentId)));
    const inboundInvoices = electronic.inboundInvoices || [];
    const connectionRows = electronic.connections.map(connection => `<article><div><strong>${escapeHtml(connection.platformLabel)}</strong><p>${escapeHtml(connectionStatusLabel(connection.status))} · ${connection.environment === "production" ? "Production" : "Bac à sable fournisseur"}</p><small>${connection.externalAccountLabel ? `Compte ${escapeHtml(connection.externalAccountLabel)} · ` : ""}${connection.lastCheckedAt ? `Vérifiée le ${escapeHtml(dateTime(connection.lastCheckedAt))}` : "Aucun test effectué"}</small></div>${connection.active ? `<div class="form-actions"><button type="button" class="secondary-button" data-test-connection="${connection.id}">Vérifier</button><button type="button" class="secondary-button" data-disconnect="${connection.id}">Déconnecter</button></div>` : ""}</article>`).join("");
    node.innerHTML = `<section class="accounting-panel">
        <div class="form-heading"><div><p class="eyebrow">Plateforme de facturation électronique</p><h3>Connexion SUPER PDP de votre entreprise</h3></div></div>
        ...
        <h4>Factures et avoirs émis</h4><div class="accounting-transmission-list">${accounting.documents.filter(item => ["invoice", "credit"].includes(item.documentType)).map(item => { const b2c = item.customerType === "Particulier"; const b2b = ["Professionnel", "Magasin"].includes(item.customerType); const alreadyTransmitted = activeTransmissionDocumentIds.has(String(item.id)); return `<article><div><strong>${escapeHtml(item.documentNumber)}</strong><p>${item.documentType === "credit" ? "Avoir" : "Facture"} · ${escapeHtml(item.customerName)} · ${money(Math.abs(item.totals.netPayable))}</p><small>${b2c ? "Particulier · B2C : facture locale et e-reporting, pas de transmission B2B" : alreadyTransmitted ? "Transmission déjà enregistrée" : b2b ? "Professionnel · transmission électronique B2B" : "Catégorie à préciser avant toute transmission B2B"}</small></div><button type="button" class="secondary-button" data-transmit="${item.id}" ${ready && b2b && !alreadyTransmitted ? "" : "disabled"}>${b2c ? "B2C · non transmissible" : alreadyTransmitted ? "Déjà transmis" : b2b ? "Transmettre" : "Catégorie à préciser"}</button></article>`; }).join("") || '<p class="muted">Aucun document disponible.</p>'}</div>
        <hr><div class="form-heading"><div><p class="eyebrow">Réception fournisseur</p><h3>Factures électroniques reçues</h3><p class="muted">Consultez d’abord la facture réellement reçue dans SUPER PDP. La récupération automatique entrante n’étant pas encore disponible dans cet adaptateur, enregistrez ensuite ici ses références et montants pour la contrôler, décider et la rapprocher d’un achat. Cet enregistrement manuel ne télécharge pas le document original depuis SUPER PDP.</p></div></div>
...
function renderSettings(node) {
    ...
    node.innerHTML = `<section class="accounting-panel">
        ...
        <form id="accountingSettingsForm" class="form-grid">
            <fieldset class="accounting-rules form-wide"><legend>Plan comptable</legend>
                ...
                <label>Compte avances / acomptes reçus<input name="customerAdvanceAccount" value="${escapeHtml(accounts.customerAdvanceAccount || "419100")}"></label>
                <label>Compte aides / franchises à recevoir<input name="aidReceivableAccount" value="${escapeHtml(accounts.aidReceivableAccount || "467000")}"></label>
                <label>Compte banque<input name="bankAccount" value="${escapeHtml(accounts.bankAccount || "512000")}"></label>
                <label>Compte caisse<input name="cashAccount" value="${escapeHtml(accounts.cashAccount || "530000")}"></label>
            ...
            <fieldset class="accounting-rules form-wide"><legend>Journaux et exercice</legend>
                ...
                <label>Code journal de caisse<input name="cashJournalCode" maxlength="10" value="${escapeHtml(journals.cash?.code || "CA")}"></label>
                <label>Libellé caisse<input name="cashJournalLabel" maxlength="100" value="${escapeHtml(journals.cash?.label || "Caisse")}"></label>
    ...
    node.querySelector("form").addEventListener("submit", async event => {
        ...
        const payload = {
            chartConfig: Object.fromEntries(["salesAccount", "customerAccount", "customerAdvanceAccount", "aidReceivableAccount", "bankAccount", "cashAccount", "vatCollectedAccount", "purchaseAccount", "supplierAccount"].map(key => [key, form.get(key)])),
            journalConfig: { sales: { code: form.get("salesJournalCode"), label: form.get("salesJournalLabel"), active: true }, bank: { code: form.get("bankJournalCode"), label: form.get("bankJournalLabel"), active: true }, cash: { code: form.get("cashJournalCode"), label: form.get("cashJournalLabel"), active: true }, general: { code: form.get("generalJournalCode"), label: form.get("generalJournalLabel"), active: true } },
