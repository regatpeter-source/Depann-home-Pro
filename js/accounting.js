...
async function renderElectronic(node) {
    ...
    node.innerHTML = `<section class="accounting-panel">
        ...
        <aside class="accounting-pdp-notice"><strong>B2B / B2C :</strong> les factures destinées aux professionnels peuvent être transmises à la plateforme connectée. Une facture destinée à un particulier reste émise, archivée et comptabilisée normalement, y compris lorsqu’elle est réglée sur place, mais elle n’est pas envoyée comme facture B2B. Son e-reporting doit être assuré auprès de la plateforme choisie par l’entreprise.</aside>
        ...
        <h4>Factures et avoirs émis</h4><div class="accounting-transmission-list">${accounting.documents.filter(item => ["invoice", "credit"].includes(item.documentType)).map(item => { const b2c = item.customerType === "Particulier"; const b2b = ["Professionnel", "Magasin"].includes(item.customerType); return `<article><div><strong>${escapeHtml(item.documentNumber)}</strong><p>${item.documentType === "credit" ? "Avoir" : "Facture"} · ${escapeHtml(item.customerName)} · ${money(Math.abs(item.totals.netPayable))}</p><small>${b2c ? "Particulier · B2C : facture locale et e-reporting, pas de transmission B2B" : b2b ? "Professionnel · transmission électronique B2B" : "Catégorie à préciser avant toute transmission B2B"}</small></div><button type="button" class="secondary-button" data-transmit="${item.id}" ${ready && b2b ? "" : "disabled"}>${b2c ? "B2C · non transmissible" : b2b ? "Transmettre" : "Catégorie à préciser"}</button></article>`; }).join("") || '<p class="muted">Aucun document disponible.</p>'}</div>
