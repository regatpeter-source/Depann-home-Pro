...
function renderSettings(node) {
    ...
    node.querySelector("form").addEventListener("submit", async event => {
        ...
        const payload = {
            chartConfig: Object.fromEntries(["salesAccount", "customerAccount", "aidReceivableAccount", "bankAccount", "cashAccount", "vatCollectedAccount", "purchaseAccount", "supplierAccount"].map(key => [key, form.get(key)])),
