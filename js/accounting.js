...
async function renderElectronic(node) {
    ...
    node.innerHTML = `<section class="accounting-panel">
        ...
        <h4>Plateformes intégrées</h4><div class="accounting-transmission-list">${electronic.providers.map(provider => `<article><div><strong>${escapeHtml(provider.label)}</strong><p>${provider.authorizationRequired ? "Autorisation OAuth sécurisée" : "Adaptateur dédié disponible"}</p><small>${provider.supports.webhooks ? "Notifications fournisseur" : "Suivi asynchrone par interrogation sécurisée"}</small></div><button type="button" class="secondary-button" data-connect="${escapeHtml(provider.code)}">${active?.platformCode === provider.code ? "Reconnecter" : "Connecter"}</button></article>`).join("") || '<p class="auth-message">Cette plateforme n\'est pas encore intégrée à Depan’Home Pro. Aucun connecteur ne sera présenté comme opérationnel sans API fournisseur documentée.</p>'}</div>
        <h4>Historique des transmissions</h4><div class="accounting-transmission-list">${accounting.transmissions.length ? accounting.transmissions.map(item => `<article><div><strong>${escapeHtml(item.documentNumber)}</strong><p>${escapeHtml(item.provider)} · ${escapeHtml(transmissionStatusLabel(item.status))}</p><small>${escapeHtml(item.remoteId || "Sans référence externe")} · ${escapeHtml(item.message || "Aucun message")}</small></div>${item.remoteId ? `<button type="button" class="secondary-button" data-refresh-transmission="${item.id}">Actualiser le statut</button>` : ""}</article>`).join("") : '<p class="muted">Aucune transmission enregistrée.</p>'}</div>
