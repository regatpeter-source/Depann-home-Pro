...
function renderStudio(shell, type, payload, onBack, preferredId = 0) {
    ...
    const inherited = type === "invoice" && !templates.some(item => item.status === "active") ? payload.inheritedFromQuote : null;
    const statusLabel = current?.status === "active" ? `Modèle ${LABELS[type]} actif` : inherited ? `Hérite du devis v${inherited.version}` : "Modèle natif actif";
    shell.innerHTML = `
        <header class="template-studio-heading"><div><p class="eyebrow">Éditeur de modèle</p><h2>${escapeHtml(capitalize(LABELS[type]))}</h2><p class="muted">${type === "invoice" ? "Sans modèle facture actif, la structure du devis actif est automatiquement réutilisée avec les données propres à la facture." : "Un seul rendu visuel est généré : votre modèle et les données placées dans ses zones."}</p></div><span class="template-studio-status ${current?.status === "active" || inherited ? "active" : ""}">${escapeHtml(statusLabel)}</span></header>
        ${inherited ? `<div class="template-inheritance-note"><strong>Héritage automatique :</strong> en-tête, zones, tableau, couleurs, marges et pied de page proviennent de « ${escapeHtml(inherited.name)} ». Numéro, date, échéance, client, lignes et totaux restent ceux de la facture.<div><button type="button" class="secondary-button" data-preview-inherited>Prévisualiser la facture héritée</button><button type="button" class="secondary-button" data-test-inherited>Tester en multipage</button></div></div>` : ""}
    ...
    shell.querySelector("[data-preview-inherited]")?.addEventListener("click", () => previewInherited(false));
    shell.querySelector("[data-test-inherited]")?.addEventListener("click", () => previewInherited(true));
    if (!current) return;
...
async function previewInherited(stress) { const response = await fetch(`/api/document-templates/invoice/inherited/${stress ? "test" : "preview"}`, { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: "{}" }); if (!response.ok) { const error = await response.json().catch(() => null); alert(error?.message || "Prévisualisation impossible."); return; } const blob = await response.blob(); const url = URL.createObjectURL(blob); const dialog = document.createElement("section"); dialog.className = "template-preview-dialog"; dialog.innerHTML = `<div><header><div><p class="eyebrow">Facture héritée du devis</p><h2>${stress ? "Test multipage" : "Prévisualisation finale"}</h2><p class="auth-message">La structure vient du devis ; toutes les données affichées sont celles d’une facture fictive.</p></div><button type="button" class="secondary-button">Fermer</button></header><iframe title="Prévisualisation de la facture héritée"></iframe></div>`; dialog.querySelector("iframe").src = url; dialog.querySelector("button").addEventListener("click", () => { URL.revokeObjectURL(url); dialog.remove(); }); document.body.append(dialog); }
function newZone(id, type, field) { return { id: `${id}-${Date.now().toString(36)}`, type, field, text: "", page: ["table", "photos", "repeatText"].includes(type) ? "all" : "first", x: 55, y: 150, width: type === "table" || type === "photos" || type === "repeatText" ? 480 : 220, height: type === "table" ? 360 : type === "photos" ? 260 : 50, style: { fontSize: 10, color: "#172033", eraseSource: type !== "fixed", backgroundColor: "#ffffff", borderColor: "#d7dde3", borderWidth: 1, align: "left", bold: false, rowHeight: 22, rows: 2, gap: 8 } }; }
