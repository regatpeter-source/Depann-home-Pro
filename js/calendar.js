...
import { createBillingDocumentForClient, viewBillingDocument } from "./billing.js?v=161";
import { getSearchableClients } from "./clients.js?v=142";
import { addClientActivityByName, synchronizeClients } from "./client-sync.js?v=123";
import { renderClientMessages } from "./messages.js?v=106";
import { renderLeakReportWizard as renderTechnicalReports } from "./leak-report-wizard.js?v=20";
...
function initializeMultiDatePlanning(form, event) {
    ...
    const render = () => {
        ...
        section.querySelector("#calendarRangeStart").value = pendingDates[0] || rangeAnchor || primaryDate;
        section.querySelector("#calendarRangeEnd").value = pendingDates.at(-1) || rangeAnchor || primaryDate;
        renderRangePicker();
    ...
    const renderRangePicker = () => {
        ...
        picker.innerHTML = `
            ...
            <div class="calendar-range-months">${months.map(month => renderRangeMonth(month, selectedDates, pendingRangeDates, rangeAnchor)).join("")}</div>
        `;
