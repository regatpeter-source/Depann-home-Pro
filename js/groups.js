...
function renderWorkspace(shell, context, dashboard = { total: {}, companies: [] }, entries, seats) {
    ...
    if (seats) {
        shell.querySelector(".group-cards").insertAdjacentHTML("afterend", `<section class="group-panel group-seat-envelope"><div class="form-heading"><div><p class="eyebrow">Enveloppe attribuée par le Créateur</p><h3>Postes à répartir dans le groupe</h3></div></div><div class="group-cards"><article><span>Entreprises</span><strong>${seats.companyCount}/${seats.maxCompanies}</strong><small>${seats.availableCompanies} disponible(s), principale incluse</small></article><article><span>Postes PC</span><strong>${seats.allocatedPcSeats}/${seats.totalPcSeats}</strong><small>${seats.availablePcSeats} à répartir</small></article><article><span>Postes mobiles</span><strong>${seats.allocatedMobileSeats}/${seats.totalMobileSeats}</strong><small>${seats.availableMobileSeats} à répartir</small></article></div><p class="muted">L’abonnement est porté par l’entreprise principale. Les allocations peuvent être rééquilibrées sans modifier la facturation, mais jamais sous les postes déjà utilisés.</p></section>`);
