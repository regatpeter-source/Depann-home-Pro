...
Dans **Console Créateur → Facturation des abonnements → Coordonnées de l’émetteur**, le Créateur choisit son régime de TVA. Le choix **Non assujetti / Franchise en base** impose une TVA à 0 % et ajoute automatiquement sur chaque nouvelle facture d’abonnement la mention **« TVA non applicable, art. 293 B du CGI »**. Le numéro de TVA intracommunautaire et un taux positif sont utilisés uniquement avec le régime **Assujetti à la TVA**. Les factures déjà émises conservent l’instantané fiscal enregistré lors de leur création. Les factures d’abonnement actives arrivées à échéance sont créées et envoyées automatiquement au démarrage du serveur puis chaque jour à l’heure configurée ; ce mécanisme n’effectue aucun prélèvement bancaire automatique.

La fiche de chaque entreprise permet aussi de configurer une réduction commerciale avec son libellé (par exemple **Offre d’essai**), soit en pourcentage, soit en montant fixe TTC. Chaque nouvelle facture détaille le nom de l’offre, le nombre de postes administratifs et de postes mobiles inclus, puis soustrait la réduction avant de calculer le net à payer. Les lignes, les quotas, le tarif et la réduction sont figés lors de la création : une modification ultérieure s’applique uniquement aux prochaines échéances.

Chaque entreprise configure de la même manière son propre régime dans **Paramètres → Coordonnées, mentions et logo**. En franchise, toutes les nouvelles lignes des devis et factures du modèle intégré sont forcées à 0 %, y compris lorsqu’un ancien modèle de prestation contenait un autre taux. Les PDF intégrés affichent automatiquement la mention légale. Si l’entreprise utilise sa propre base Word ou PDF externe, elle reste responsable de la présence de cette mention dans son fichier personnalisé.
