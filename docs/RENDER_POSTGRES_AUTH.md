...
Sur la fiche d’un client et dans le registre commercial, chaque devis ou facture dispose de **PDF / Imprimer** : un fichier PDF privé est généré par le serveur et s’ouvre dans le visualiseur de l’appareil, qui permet l’aperçu, le téléchargement et l’impression. L’action **Envoyer le PDF** joint directement ce fichier à un e-mail envoyé au client. Elle requiert l’adresse du client dans sa fiche ainsi que les variables SMTP décrites dans `.env.example` ou dans l’environnement Render. La fiche présente aussi un **historique** daté : création ou modification du dossier, ajout/suppression de fichiers, nouveaux rendez-vous, devis et factures. Cet historique fait partie du dossier client privé et se synchronise avec les autres appareils du même compte.
