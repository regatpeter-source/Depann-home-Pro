...
La double authentification (2FA) d’entreprise est une protection TOTP optionnelle, activée par un **Administrateur administratif** dans **Paramètres → Sécurité**. Elle concerne uniquement les comptes dont le rôle est `admin`.

Elle ne change pas les flux d’accès des Administrateurs Mobile, postes administratifs standard, comptables, techniciens ou chefs d’équipe, ni l’activation existante des smartphones.
...
1. Un Administrateur administratif active la politique entreprise.
2. À la prochaine connexion d’un Administrateur administratif, après identifiant et mot de passe, le serveur présente un QR code et une clé manuelle.
...
Tout Administrateur administratif authentifié peut réinitialiser l’authentificateur d’un Administrateur administratif de sa propre entreprise depuis la section Sécurité. L’authentificateur est supprimé ; le titulaire recevra un nouveau QR code à sa prochaine connexion.

Cette première version ne fournit volontairement ni code de secours ni deuxième appareil. Si tous les Administrateurs administratifs perdent simultanément leur application et aucune session ne reste active, une intervention du support reste nécessaire. Le modèle de table permet l’ajout ultérieur de ces mécanismes sans migration conceptuelle.
