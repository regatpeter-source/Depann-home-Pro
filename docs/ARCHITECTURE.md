...
Le Créateur choisit et peut modifier à tout moment le type d’interface (`partner`, `standard` ou `group`), le secteur d’activité et la licence (`partner_portal`, `depannhome_standard` ou `depannhome_group`). Les droits effectifs sont calculés par `server/organizations.js`, ajoutés à la session authentifiée et vérifiés côté interface comme côté API pour les modules sensibles. Les organisations Partenaire conservent les clients, les missions, la collaboration, la recherche d’entreprises dans le Réseau Depann’Home Pro et le formulaire Support. Leur hub Paramètres expose uniquement Réseau et Support. Elles n’accèdent jamais aux connecteurs, accès API externes ou environnements de recette réservés aux donneurs d’ordre, ni aux modules techniques, comptables ou d’importation. Les interfaces Standard et Groupe conservent l’expérience complète, sous réserve des droits de rôle et d’appareil existants.
...
- Les aides, règlements, paramètres comptables, connexions de plateforme et historiques de transmission sont persistés dans des tables dédiées.
...
- Sur poste administratif, la création et la modification d’un devis ou d’une facture utilisent un atelier immersif à deux volets : formulaire complet à gauche et aperçu PDF actualisé à droite après chaque série de saisies. L’aperçu accepte temporairement un brouillon incomplet avec des libellés d’attente, mais la route d’enregistrement conserve les validations strictes habituelles. La route `POST /api/billing/documents/preview` ne persiste aucune donnée et refuse les appareils mobiles. Un gabarit PDF externe est fusionné dans cet aperçu comme dans le document final ; lorsqu’une entreprise utilise un gabarit DOCX, le navigateur affiche les pages métier PDF et signale que le DOCX sera appliqué lors de l’enregistrement ou de l’export.
...
- Une facture reste un brouillon sans numéro fiscal jusqu’à son émission définitive. L’émission attribue transactionnellement un numéro annuel continu, fige un instantané légal et archive le PDF ainsi que sa représentation UBL EN 16931 avec leurs empreintes SHA-256. Les avoirs suivent la même chaîne lors de leur création. Un déclencheur PostgreSQL interdit ensuite la modification ou la suppression de leurs données légales et archives.
- Depann'Home Pro prépare les factures électroniques mais n'est ni une plateforme agréée, ni une PDP, ni une plateforme certifiée de transmission. SUPER PDP est l’unique plateforme utilisable et chaque entreprise détient sa propre connexion.
- Aucun protocole universel n’est supposé. Un registre d’adaptateurs fournisseur couvre connexion, facture, avoir, statut, renouvellement et informations de compte selon l’API officiellement documentée. Sans adaptateur réel, la plateforme est indiquée comme non intégrée. Les connexions et credentials chiffrés sont isolés par `owner_id`; les webhooks résolvent ce tenant par un identifiant opaque de connexion plutôt que par une valeur fournie par l’appelant.
- Le contrat du transport et les étapes de raccordement sont détaillés dans `docs/ELECTRONIC_INVOICING.md`.
...
Le rendu PDF est uniformisé par `server/leak-report-template.js` : en-tête d’entreprise, titre centré, séparateur coloré, pied de page et pagination sont apposés à toutes les pages. Sur poste administratif, l’administrateur peut ouvrir **Modifier le modèle de rapport** depuis l’espace Devis/Factures ou la liste des rapports ; ce bouton mène à **Paramètres → Rapports → Modèle de rapport**. Le gabarit est enregistré une seule fois par entreprise dans `depannhome_billing_profiles.report_template` : logos principal et secondaire, identité et coordonnées, textes, couleurs, police et éléments du pied de page. `GET` et `PUT /api/technical-reports/template` sont réservés à l’administration du rapport. La dernière version est appliquée à tous les brouillons, prévisualisations et futurs PDF de l’entreprise ; un PDF déjà validé reste archivé et immuable. Ces préférences ne modifient pas les règles de verrouillage, validation ou archivage.
