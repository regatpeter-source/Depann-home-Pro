...
-- Assistance Créateur temporaire : une session identifie le motif et le cadre
-- d'accès ; chaque réparation conserve son état avant/après sans aucun secret.
...
-- Équipes personnalisées propres à chaque entreprise (ou siège actif d’un groupe).
-- Les sections métier restent indépendantes et peuvent être partagées par plusieurs équipes.
...
CREATE UNIQUE INDEX IF NOT EXISTS depannhome_teams_owner_name_unique ON depannhome_teams(owner_id, LOWER(name));
CREATE INDEX IF NOT EXISTS depannhome_teams_owner_active_idx ON depannhome_teams(owner_id, is_active, LOWER(name));
...
CREATE INDEX IF NOT EXISTS depannhome_team_memberships_member_idx ON depannhome_team_memberships(member_id, team_id);
CREATE OR REPLACE FUNCTION depannhome_validate_team_membership() RETURNS trigger AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM depannhome_teams team JOIN depannhome_users member ON member.id=NEW.member_id
...
CREATE TRIGGER depannhome_team_membership_company BEFORE INSERT OR UPDATE ON depannhome_team_memberships
...
CREATE INDEX IF NOT EXISTS depannhome_users_active_trial_end_idx ON depannhome_users(trial_ends_at) WHERE subscription_status='trial' AND is_archived=FALSE;
...
-- Chaque remise en brouillon fige la version validée précédente. Ces copies
-- restent consultables et ne peuvent être ni réécrites ni supprimées.
...
CREATE INDEX IF NOT EXISTS depannhome_technical_report_originals_report_idx ON depannhome_technical_report_originals (owner_id, report_id, revision DESC);
...
CREATE TRIGGER depannhome_technical_report_original_immutable BEFORE UPDATE OR DELETE ON depannhome_technical_report_originals FOR EACH ROW EXECUTE FUNCTION depannhome_protect_technical_report_original();
...
-- Reçu minimal des purges volontaires de journaux opérationnels. Le contenu
-- supprimé n'est jamais recopié dans ce journal.
...
CREATE TABLE IF NOT EXISTS depannhome_partner_email_messages (
    ...
    subject VARCHAR(500) NOT NULL DEFAULT '', body_text TEXT NOT NULL DEFAULT '', document_text TEXT NOT NULL DEFAULT '', document_extraction_version INTEGER NOT NULL DEFAULT 0, received_at TIMESTAMPTZ NOT NULL,
